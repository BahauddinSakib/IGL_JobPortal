...
const CreateJobSkillPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // alert('Job skill created successfully!');
        router.push('/admin/job-attributes/job-skills');
      } else {
        alert(result.error || 'Error creating job skill');
      }
    } catch (error) {
      console.error('Error:', error);
      alert('Error creating job skill');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container-fluid p-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1 className="h3 mb-0 text-dark fw-bold">CREATE JOB SKILL</h1>
        <button
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-header bg-light">
          <h5 className="mb-0 text-dark fw-bold">Add New Job Skill</h5>
        </div>
...
export default CreateJobSkillPage;