...
module.exports.updateCategory = async (req, res, next) => {
  try {
    ...
    const currentCategory = await db('pro_category').where({ id }).first();
    if (!currentCategory) {
      return res.status(404).json({ message: 'Category not found' });
    }
    ...
    const updateData = {
      ...
      pro_cat_content_description: content_description,
      pro_cat_meta_keys: cat_meta_keys,
      pro_cat_parent_id: parent_id || null,
      updated_at: db.fn.now()
    };

    // Step 3: If a new image is uploaded, delete old one and update filename
    if (cate_img) {
      if (currentCategory.pro_cat_img) {
        const oldImagePath = path.join(categoryDataFolder, currentCategory.pro_cat_img);
    ...
    const updatedRows = await db('pro_category').where({ id }).update(updateData);
    if (updatedRows === 0) {
      return res.status(404).json({ message: 'Category not updated' });
    }

    // Step 5: Return updated category
    const updatedCategory = await db('pro_category').where({ id }).first();
