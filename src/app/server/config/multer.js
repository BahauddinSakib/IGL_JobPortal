...
const compressSiteDataImages = async (req, res, next) => {
  ...
  try {
    ...
    console.warn(`Failed to delete ${originalPath}:`, err.message);
  }
}
    ...
    for (const file of files) {
      await compressAndReplace(
        file,
        file.fieldname === "sd_banner" ? 1600 : 100,
        file.fieldname === "sd_logo" ? 100 : 400
      );
    }
...
const compressCategoryImage = async (req, res, next) => {
  try {
    const file = req.files?.cate_img?.[0];

    if (!file || !file.path) return next();

    const originalPath = file.path;
    const baseName = path.parse(file.filename).name;
    const webpPath = path.join(categoryDataFolder, `${baseName}.webp`);
...
// vendor  upload setting
const vendorFolder = path.join(__dirname, "..", "../uploads", "vendor-documents");
