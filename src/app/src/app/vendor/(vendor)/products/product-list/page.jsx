...
const page = () => {
  ...
  useEffect( ()=>{
    const fetchData = async ()=>{
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/products/vendor-product-list/${user?.id}`, { withCredentials: true,});
			...
			<div className="ec-content-wrapper">
				<div className="content">
					<div className="breadcrumb-wrapper d-flex align-items-center justify-content-between">
						...
						<div>
							<Link href="/vender/products/add-product" className="btn btn-primary"> Add Porduct</Link>
						</div>
					</div>
					<div className="row">
  ...
  <Link 
    href={`/vendor/products/product-details/${product.id}`} 
  ...
  <div className="dropdown-menu">
    ...
    <Link 
      href={`/vendor/products/edit-product/${product.id}`} 
      className="dropdown-item"
    >
      Edit Product
    </Link>
    ...
    <Link 
      href={`/vendor/products/delete/${product.id}`}
